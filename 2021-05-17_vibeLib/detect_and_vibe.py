...
# 转换坐标，与原图大小相对应
def detection_adjust(detections):
    detections_adjusted=[]
...
def isMove(frame,detections,frameDelta,thresh,class_colors):
    ...
    """
    # 画检测结果的边界框
    detections_adjusted=detection_adjust(remain_detections)
    image = darknet.draw_boxes(detections_adjusted, frame, class_colors)
    # cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (255,0,0), 1)
    # cv2.putText(image, "suppression", (xmax, ymax - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0,255,0), 1)
    ...
    for index in range(0, frame_total, batch_size):
        ...
        while not frame_queue.empty():
            ...
            if len(detections) != 0 :
                ...
                image=isMove(frame,detections,frameDelta,0.01,class_colors)
            else:
                image=frame
