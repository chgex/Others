...
        每个像素的样本集numpy数组
    ...
    def Update(self,img):
        ...
        #前景像素设置为100,其实可以是任何非零值，表示前景像素不需要更新样本集
        upfactor[matches]=100
        #满足更新自己样本集像素的索引
        upSelfSamplesInd=np.where(upfactor==0) 
        #生成随机更新自己样本集的的索引
        upSelfSamplesPosition=np.random.randint(self.defaultNbSamples,size=upSelfSamplesInd[0].shape)
